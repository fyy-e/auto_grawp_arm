...
DOF6Kinematic::DOF6Kinematic(float L_BS, float D_BS, float L_AM, float L_FA, float D_EW, float L_WT)
    : armConfig(ArmConfig_t{L_BS, D_BS, L_AM, L_FA, D_EW, L_WT})
{
    // 1. 初始化DH矩阵（严格对应你的DH参数表）
    float tmp_DH_matrix[6][4] = {
        // 关节1: theta=0, d=0.1525, a=0.0, alpha=pi/2
        {0.0f,            armConfig.L_BASE,    armConfig.D_BASE, (float)M_PI_2},
        // 关节2: theta=pi/2, d=0.0, a=0.0, alpha=-pi/2
        {(float)M_PI_2,   0.0f,                0.0f,             -(float)M_PI_2},
        // 关节3: theta=0, d=0.17, a=0.0, alpha=pi/2
        {0.0f,            armConfig.L_ARM,     0.0f,             (float)M_PI_2},
        // 关节4: theta=0, d=0.0, a=0.0, alpha=-pi/2
        {0.0f,            0.0f,                0.0f,             -(float)M_PI_2},
        // 关节5: theta=0, d=0.12, a=0.0, alpha=0
        {0.0f,            armConfig.L_FOREARM, 0.0f,             0.0f},
        // 关节6: theta=0, d=0.0, a=0.0, alpha=0
        {0.0f,            armConfig.L_WRIST,   0.0f,             0.0f}
    };
    memcpy(DH_matrix, tmp_DH_matrix, sizeof(tmp_DH_matrix));

    // 2. 初始化链接向量（从{i-1}坐标系到{i}坐标系的原点向量）
    // 基座(0)→关节1(1): [A1, 0, D1] = [0.0, 0.0, 0.1525]
    float tmp_L1_bs[3] = {armConfig.D_BASE, 0.0f, armConfig.L_BASE};
    memcpy(L1_base, tmp_L1_bs, sizeof(tmp_L1_bs));
    // 关节1(1)→关节2(2): [A2, 0, D2] = [0.0, 0.0, 0.0]
    float tmp_L2_arm[3] = {0.0f, 0.0f, 0.0f};
    memcpy(L2_arm, tmp_L2_arm, sizeof(tmp_L2_arm));
    // 关节2(2)→关节3(3): [A3, 0, D3] = [0.0, 0.0, 0.17]
    float tmp_L3_elbow[3] = {0.0f, 0.0f, armConfig.L_ARM};
    memcpy(L3_elbow, tmp_L3_elbow, sizeof(tmp_L3_elbow));
    // 关节5(5)→关节6(6): [A6, 0, D6] = [0.0, 0.0, 0.0]
    float tmp_L6_wrist[3] = {0.0f, 0.0f, armConfig.L_WRIST};
    memcpy(L6_wrist, tmp_L6_wrist, sizeof(tmp_L6_wrist));

    // 3. 初始化逆运动学相关参数（适配你的DH参数）
    l_se_2 = armConfig.L_ARM * armConfig.L_ARM;  // 0.17*0.17=0.0289
    l_se = armConfig.L_ARM;                      // 0.17
    l_ew_2 = armConfig.L_FOREARM * armConfig.L_FOREARM + armConfig.D_ELBOW * armConfig.D_ELBOW; // 0.12²+0=0.0144
    l_ew = sqrt(l_ew_2);                         // 0.12
    atan_e = atan2(armConfig.D_ELBOW, armConfig.L_FOREARM); // atan2(0,0.12)=0
}
